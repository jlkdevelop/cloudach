...
/**
 * In-process rate limiter for auth endpoints (login, register).
 * Tracks request counts per IP in a Map. Entries expire after `windowMs`.
 * Note: per-instance — not shared across serverless replicas. For distributed
 * enforcement, layer Vercel/CDN-level rate limiting in front of these routes.
 *
 * Returns a `check(ip, res)` function. If the limit is exceeded, it writes
 * a 429 response and returns true. Otherwise it returns false.
 */
export function createAuthRateLimiter(limit, windowMs) {
  // Map<ip, { count: number, resetAt: number }>
  const store = new Map();

  return function check(ip, res) {
    const now = Date.now();
