...
<head>
  ...
  <style>
    ...
    .body { padding: 32px; }
    .badge { display: inline-block; padding: 4px 12px; border-radius: 20px; font-size: 13px; font-weight: 700; background: ${thresholdPct >= 100 ? '#FEE2E2' : thresholdPct >= 80 ? '#FEF3C7' : '#EFF6FF'}; color: ${thresholdPct >= 100 ? '#991B1B' : thresholdPct >= 80 ? '#92400E' : '#1D4ED8'}; margin-bottom: 16px; }
    ...
    .bar-fill { height: 100%; border-radius: 4px; background: ${thresholdPct >= 100 ? '#DC2626' : thresholdPct >= 80 ? '#F59E0B' : '#4F6EF7'}; width: ${Math.min(thresholdPct, 100)}%; }
    ...
    .cta { display: inline-block; margin-top: 8px; padding: 10px 20px; background: #4F6EF7; color: #fff; text-decoration: none; border-radius: 8px; font-size: 14px; font-weight: 600; }
