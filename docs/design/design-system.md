...
text-align: center
padding: 48px 24px
icon: 36–40px SVG (not emoji — see Icon Library below)
title: 15px 600 --color-text-muted
description: 13px --color-text-subtle
CTA button below description (required)
```
...
Persistent progress card at top of overview page:
- White card, 12px radius, 1px border
- 3px progress bar at top (brand color fill, gray track)
- Header: "Get started with Cloudach" + "N of 3 done" count
- Step rows: 20px check circle (gray → brand active → green done) + label + chevron/CTA
- Completed steps: strikethrough label, green check
- Active step: auto-expanded body with description, optional code snippet, CTA button
- Dismiss link at bottom; state stored in localStorage
- Auto-hides when all steps complete (shows success banner 24h then dismisses)
...
| Chart empty | `pages/dashboard/index.jsx:IconChartEmpty` | Usage empty state |
...
| Align to 4/8px grid | Use arbitrary spacing values |
| Use `box-shadow` focus rings with brand color | Use browser default outlines on inputs |
